...
\begin{figure}[htbp]
    ...
    \begin{minipage}[c]{0.25\linewidth}
        \centering
        \includegraphics[width=\linewidth]{exp1-电子逸出功/实验仪器-金属电子逸出功.png}
        \caption{金属电子逸出功实验仪}
    \end{minipage}
    \hspace{1em} % 添加一点水平间距，防止图片紧贴
    \begin{minipage}[c]{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{exp1-电子逸出功/实验仪器-电子管综合实验仪.png}
        \caption{电子管综合实验仪}
    \end{minipage}
\end{figure}
...
\begin{minipage}[t]{0.35\linewidth} % 右侧图片部分，占 30% 宽度
    ...
    \centering
    \includegraphics[width=\linewidth]{exp1-电子逸出功/图1.2a.png}
\end{minipage}

\newcolumntype{Y}{>{\centering\arraybackslash}X}
\begin{table}[htbp]
    \centering
    % 使用 \renewcommand{\arraystretch}{1.2} 增加行高，让表格不那么拥挤
    \renewcommand{\arraystretch}{1.2} 
    \caption{不同灯丝电流 $I_f$ 和阳极电压 $U_a$ 对应的阳极电流 $I_a$ 值} \label{table1}
    
    % 如果列数固定，有时直接用 tabular 比 tabularx 更容易控制间距
    \begin{tabularx}{\textwidth}{c *{7}{Y}}
        ...
        0.700 & & & & & & & \\ 
        \bottomrule
        \multicolumn{8}{l}{\small 注：表中数据单位为 mA。} % 如果 caption 太长，可以在这里注明单位
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{ $\lg I/T^2\sim 1/T$ 图线相关数据} \label{table2}
